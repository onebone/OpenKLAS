...
class AppProvidesModule {
    ...
    fun provideObjectMapper(configs: Set<@JvmSuppressWildcards RWJacksonConfig>): ObjectMapper {
        ...
        deserializationFeatureMap.putAll(configs.map { it.getDeserialzationFeaturePairs() }.flatten())
    ...
    @Provides
    fun provideRetrofit(okHttpClient: OkHttpClient, gson: Gson, objectMapper: ObjectMapper): Retrofit {
