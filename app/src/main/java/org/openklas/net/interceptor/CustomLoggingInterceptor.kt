...
class CustomLoggingInterceptor(private val logger: HttpLoggingInterceptor.Logger, private val thresholdBinary: Int) : Interceptor {
    @Volatile var level: HttpLoggingInterceptor.Level = HttpLoggingInterceptor.Level.NONE
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (logHeaders) {
            ...
            for (i in 0 until headers.size()) {
                ...
                if (!CONTENT_TYPE.equals(name, ignoreCase = true) && !CONTENT_LENGTH.equals(name, ignoreCase = true)) {
        ...
        logger.log("<-- ${response.code()} ${if (response.message().isEmpty()) "" else ' ' + response.message()}" +
